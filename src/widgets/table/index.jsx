...
export default class Table extends React.Component {
  ...
  getLocale() {
    let locale = {};
    if (this.context.shieldLocale && this.context.shieldLocale.Table) {
      locale = this.context.shieldLocale.Table;
    }
    ...
    if (this.props.rowSelection) {
      ...
      if (this.props.rowSelection.type === 'radio') {
        selectionColumn = {
          ...
          className: 'shield-table-selection-column',
        };
      } else {
    ...
    return columns.map((originColumn, i) => {
      ...
      if (column.sorter) {
        ...
        if (isSortColumn) {
          ...
          if (sortOrder) {
            column.className += ' shield-table-column-sort';
          }
        }
        const isAscend = isSortColumn && sortOrder === 'ascend';
        const isDescend = isSortColumn && sortOrder === 'descend';
        sortButton = (
          <div className="shield-table-column-sorter">
            <span className={`shield-table-column-sorter-up ${isAscend ? 'on' : 'off'}`}
              title="↑"
              onClick={() => this.toggleSortOrder('ascend', column)}>
              <Icon type="caret-up" />
            </span>
            <span className={`shield-table-column-sorter-down ${isDescend ? 'on' : 'off'}`}
              title="↓"
  ...
  render() {
    ...
    const classString = classNames({
      [`shield-table-${this.props.size}`]: true,
      'shield-table-bordered': this.props.bordered,
    });
    ...
    if (!data || data.length === 0) {
      emptyText = (
        <div className="shield-table-placeholder">
          {locale.emptyText}
        </div>
      );
      emptyClass = 'shield-table-empty';
    }
    ...
    const paginationPatchClass = (this.hasPagination() && data && data.length !== 0)
            ? 'shield-table-with-pagination'
            : 'shield-table-without-pagination';
    const spinClassName = this.props.loading ? `${paginationPatchClass} shield-table-spin-holder` : '';
